# ================================
# BERT Sentiment Analysis on Tweets
# ================================#epoch = 10
...
def clean_text(text):
    ...
    return text
    # NOTE: NOT remove stopwords,   punctuation, or emojis.
    # NOT lemmatizing.
...
def train_bert_model(df):
    ...
    def compute_metrics(p):
        preds = np.argmax(p.predictions, axis=1)  #choose max value of each class logit (index)
...
def evaluate_model(trainer, test_df):
    ...
    preds_output = trainer.predict(trainer.eval_dataset)  #run test data
    y_pred = np.argmax(preds_output.predictions, axis=1)  #max value of each class logit
    ...
    plt.show()
    #conducted error analysis as per personal curiousity
