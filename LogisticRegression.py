#LogisticRegression run on VScode
import pandas as pd
import numpy as np
import re
import nltk
...
# Remove negation words from the "ban list" so we keep them in the text
...
def train_model(df):
    ...
    pipeline = Pipeline([
        ...
        # FIX APPLIED: Changed solver to 'lbfgs' to handle 3+ classes (Positive/Negative/Neutral)
...
def evaluate_model(model, X_test, y_test):
    ...
    # --- NEW: Show First 30 Results ---
    print("\n" + "="*30)
    print("   FIRST 30 PREDICTIONS   ")
    print("="*30)
    
    # Create a DataFrame to make viewing easy
    results_df = pd.DataFrame({
        'Text (Processed)': X_test,
        'Actual': y_test,
        'Predicted': y_pred
    })
    
    # Reset index so it counts 0, 1, 2... instead of random row numbers
    results_df.reset_index(drop=True, inplace=True)
    
    # Print the first 30 rows nicely
    # We use to_string() so pandas doesn't hide columns or truncate too much
    print(results_df.head(30).to_string())
    
    # Optional: Save to CSV if you want to inspect all of them in Excel
...
if __name__ == "__main__":
    ...
    if df is not None:
         # --- NEW: Visualize the Class Count ---
        print("Visualizing data distribution...")
        visualize_sentiment_distribution(df)
        model, X_test, y_test = train_model(df)
        
        # This function prints the Accuracy and Report you asked for
        evaluate_model(model, X_test, y_test)
        
        # Quick test
        print("\n--- Quick Custom Test ---")
        sample_tweets = ["This is amazing!", "This is terrible.", "I am okay with this."]
        preds = model.predict([clean_text(t) for t in sample_tweets])
